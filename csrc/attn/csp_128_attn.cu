...
// static constexpr int NUM_CONSUMER_REGISTERS = 144;
// static constexpr int NUM_PRODUCER_REGISTERS = 80;
// static constexpr int KV_TILE_ROWS = 96;

static constexpr int NUM_CONSUMER_REGISTERS = 160;
static constexpr int NUM_PRODUCER_REGISTERS = 32;
static constexpr int KV_TILE_ROWS = 128;

static constexpr int INDICES_LOAD_INTERVAL = 8;
static constexpr bool CACHE_SWIZZLE_OFFSETS = true;
...
static constexpr bool TMA_KV = false;
...
template<int D, int NUM_WORKERS> struct attn_fwd_layout {
    ...
    struct globals { qo_global O, Q; kv_global K, V; int *indices, *indices_counts; };
...
template<int D> struct attn_fwd_template {
    ...
    __device__ static inline void common_setup(common_setup_args<layout> args) {
        // todo fix when we have more than 1 consumer - should also be consumer dependent
        ...
        // num_iters = (num_iters + layout::kv_tile::rows - 1) / layout::kv_tile::rows;
    ...
    struct producer {
        ...
        __device__ static inline void load_async_gather(ST &dst, ST &dst2, const GL &src, const GL &src2, const COORD &idx, int iter, uint32_t *swizzle_offsets, int *s_indices) {
            using T = typename ST::dtype;
            ...
            typename GL::dtype *src2_ptr = (typename GL::dtype*)&src2[unit_coord];
            ...
            // tma1d(128b swizzle, 1024 repeating)
            // 4 contig columns
            ...
            for(int i = 0; i < total_calls; i++) {
                ...
                asm volatile(
                    ...
                    :: "r"(dst_ptr + swizzle_offset), "l"(&src_ptr[global_row*row_stride + col])
                    : "memory"
                );

                asm volatile(
                    "cp.async.cg.shared.global [%0], [%1], 16;\n"
                    :: "r"(dst2_ptr + swizzle_offset), "l"(&src2_ptr[global_row*row_stride + col])
                    : "memory"
                );
            }
        }
        ...
        __device__ static inline void load(producer_load_args<layout> args) {
            if (TMA_KV) {
                ...
                // (2) Load K and V.
                load_async_gather<2, CACHE_SWIZZLE_OFFSETS>(args.input.k, args.input.v, args.globals.K, args.globals.V, {args.common.batch, args.common.head, 0, 0}, args.iter, args.state.swizzle_offsets, &(args.scratch.indices[0]));
...
namespace chipmunk {
torch::Tensor csp_128_attn(torch::Tensor q, torch::Tensor k, torch::Tensor v, torch::Tensor indices, torch::Tensor indices_counts)
{
    ...
    ker_template::layout::globals globals {Og, Qg, Kg, Vg, d_indices, d_indices_counts};
    ...
    // if (seq_len > MAX_SUPPORTED_ATTN_N) {
    //     throw std::runtime_error("Sequence length must be less than MAX_SUPPORTED_ATTN_N");
    // }
